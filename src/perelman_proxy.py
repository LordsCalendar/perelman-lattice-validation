...
# Numerical evolution proxy (perturbed R0=6.0001, t_span 0 to 12.49 s, 33 steps) - scipy solve_ivp with mpmath converted
def ricci_evol(t, y):
    R = mp.mpf(y[0])
    R_eq = mp.mpf('6')
    gamma_2malpha = mp.gamma(2 - alpha)
    D_f = (R - R_eq) / gamma_2malpha * mp.power(mp.mpf(t) + mp.mpf('1e-10'), 1 - alpha) # Proxy on deviation only
    dR_dt = -mp.mpf('2') * (R - R_eq) + delta * D_f # Damping toward S^3 baseline
    return [float(dR_dt)]
t_span = (0, 12.49) # τ = 33 * t15 ≈12.49 s
y0 = [float(mp.mpf('6.0001'))] # Tiny perturbed initial R
...
psi0 = tensor((basis(2,0) + basis(2,1)).unit(), (basis(2,0) + basis(2,1)).unit())  # Bell-like superposition proxy (normalized)
times = np.linspace(0, 12.49, 34) # 33 steps
result = mesolve(H, psi0, times, c_ops=[], e_ops=[tensor(sigma_x, sigma_x)], options={'store_states': True}) # Concurrence proxy via <σ_x ⊗ σ_x>
